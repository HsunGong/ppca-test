...
AP common(int A0, int B0, int A1, int B1, const vi &curp)
{
    ...
    for (auto p : curp)
    {
        ...
        if (intersection0.s < 0 || intersection1.s < 0)
            return AP(-1, -1);
    }
...
int stupid_solve(vi a, vi b)
{
    ...
    for (auto p : primesa[0])
        if (!binary_search(primesb[0].begin(), primesb[0].end(), p))
        {
            for (int i = 1; i < a.size(); ++i)
            {
                if (ord(a[i], p) != ord(a[i - 1], p))
                    return -1;
            }
        }
